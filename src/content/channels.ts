...
export const architectureArgues = {
  ...
  body: [
    'The valve is the whole design. Everything else follows from one rule: analysis may consume facts, and facts may not be replaced by analysis. Take the rule away and the three channels collapse into one system that is either candid and exposed, or protected and useless.',
    'Notice what the machine never does. It never hides a fact, never deletes anything, and never claims protection for work the firm would have done anyway. It withholds the legal advice, and the candour of the first channel is what makes that claim credible.',
  ],
} as const
