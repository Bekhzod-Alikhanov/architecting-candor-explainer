...
export const linterArgues = {
  ...
  body: [
    'The distinction it enforces is not a litigation trick. As the causal-assertion rule above says, an explanation written at the moment of observation is rarely supported by controlled evidence — so the factual entry is both the legally safer record and the more accurate one, and the question it defers is answered in the channel built to answer it.',
    'It is also the only part of this architecture that costs nothing to adopt. A firm that never builds a tripwire, never separates a channel and never speaks to its general counsel can still template its ticket fields, and that removes the single most common way an engineering record turns into an admission.',
  ],
} as const
