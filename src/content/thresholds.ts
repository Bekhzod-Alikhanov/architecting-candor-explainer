...
export const calibrateArgues = {
  ...
  body: [
    'No setting makes all four readouts good at once, and that is the finding rather than a limitation of the model. Bands high enough to keep counsel’s involvement rare are high enough to miss real signals; bands low enough to catch everything turn the privileged channel into ordinary business process and forfeit the protection it exists for.',
    'The tiered structure is the only move that improves two readouts without worsening a third, because it separates capturing a record from engaging counsel. That separation is also what makes the trigger defensible: a threshold set before any harm-framed record exists reads as pre-committed rather than as a reactive assertion made once the firm knew it had a problem.',
  ],
} as const
